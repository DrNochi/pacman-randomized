import Map, { doormatTile } from './map.js'
...
var genRandom = function () {
    ...
    var isOpenCell = function (cell, i, prevDir, size) {
        ...
        if (size == 2 && (i == prevDir || direction.reverse(i) == prevDir)) {
            return false;
        }
    ...
    var connectCell = function (cell, dir) {
        ...
        cell.next[dir].connect[direction.reverse(dir)] = true;
...
var makeExitPaths = function (map) {
    ...
    var graph = getShortestDistGraph(map, doormatTile.x, doormatTile.y, isNodeTile);
    ...
    map.getExitDir = function (x, y) {
        ...
        if (dirEnum != undefined) {
            return direction.reverse(dirEnum); // reverse direction (door->ghost to door<-ghost)
        }
    };
};
...
var makeFruitPaths = (function () {
    ...
    var dirChars = {};
    dirChars[direction.up] = '^';
    dirChars[direction.down] = 'v';
    dirChars[direction.left] = '<';
    dirChars[direction.right] = '>';
...
export function mapgen() {
    ...
    map.wallStrokeColor = colors.rgbString(colors.hslToRgb(Math.random(), Math.random(), Math.random() * 0.4 + 0.6));
